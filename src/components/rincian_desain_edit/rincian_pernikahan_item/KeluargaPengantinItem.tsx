...
const FamilyColumn = ({ gender, editData, updateData }: GenderType) => {
  ...
  const anakKe = isPria
    ? editData?.anakKeBerapaPria
    : editData?.anakKeBerapaWanita
  const jmlSaudara = isPria
    ? editData?.jmlSaudaraPria
    : editData?.jmlSaudaraWanita
  const setParent = (
  ...
  return (
    <>
      ...
      <input
        ...
        value={ayahValue}
        onChange={(e) => setParent(e, true)}
      />
      <input
        type="text"
        placeholder="Nama ibu/wali"
        value={ibuValue}
        onChange={(e) => setParent(e, false)}
      />
      <input
        type="text"
        placeholder={`Pengantin ${gender} anak ke berapa`}
        value={anakKe}
        onChange={(e) => setAnakKe(e, true)}
      />
      <label className="ex_label_family">Contoh: pertama</label>
      <input
        type="text"
        placeholder={`Jumlah saudara pengantin ${gender}`}
        value={jmlSaudara}
        onChange={(e) => setAnakKe(e, false)}
      />
